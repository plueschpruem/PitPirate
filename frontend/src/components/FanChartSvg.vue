...
const yStep        = computed(() => niceStep(Y_MAX - Y_MIN, yTargetTicks.value))
const yRange       = Y_MAX - Y_MIN   // always 100

const yTicks = computed(() => {
	const ticks: number[] = []
	for (let v = Y_MIN; v <= Y_MAX + 1e-9; v += yStep.value) {
...
function yPx(val: number): number {
	return PAD_TOP + plotH.value - ((val - Y_MIN) / yRange) * plotH.value
}
...
async function load(showSpinner = true) {
	...
	try {
		...
		const res = await fetch(
			`${base}/php/telemetry_get.php?probe=fan${rangeParam}&_=${Date.now()}`
		)
...
<template>
	<div ref="wrapperEl" class="probe-chart pchart" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
		...
		<svg v-else class="pchart__svg" :width="containerWidth" :height="svgH">
			<defs>
				<clipPath id="pchart-clip-fan">
					<rect :x="PAD_LEFT" :y="PAD_TOP" :width="plotW" :height="plotH" />
				</clipPath>
				<linearGradient id="pchart-grad-fan" x1="0" y1="0" x2="0" y2="1">
			...
			<g class="pchart__plot" clip-path="url(#pchart-clip-fan)">
				<path class="pchart__area" :d="areaPath" fill="url(#pchart-grad-fan)" />
