...
- Frontend: vue3 (TS) + Vuetify
- Backend (server): PHP
- Front- & Backend compilation / packing: Vite
...
- 🔲 Multi-device support (more than one thermometer).
- 🔲 All those **super important** extra settings like _Meat Type_ or device native alarms seem to require Tuya cloud. We don't need them anyway.
...
Just as every other IoT device today the NC01 BBQ thermometer requires you to use a dedicated app that only runs on your cellphone.  
...
Since I do not use any other home automation, I also don't have much insight into them. So if I miss anything here, tell me in the comment section. That also means i have zero inclination to implement HomeAssistant or similar frameworks. If that even makes sense.
...
13. I was about to write how you might obtain the device's local IP (Tuya cloud only has the public IP), but remembered how the device regularly blasts UDP beacons on port 6667 into your LAN anyway.  
...
- My 2432S028Rv3 board misses two 5.1k resistors on the USB-C CC1 and CC2 lines.  
  That mean it works fine with the USB-C > USB-A that is included.  
  but with a USB-C <—> USB-C cable it doesn't work at all!  
  So it fails to trigger the 5V power supply, preventing the board from turning on or being recognized.  
...
- When switching probes on the NC01, the device will from there on out return the last know value for the empty probe slot/s.  
  ...
  1. Press the **Reset** button on the back of the NC01
  2. Pull out any probe and re-inset it. This will trigger a status update and the ESP32 will know what to show and what not.

- The NC01 only knows four battery states: 100% / 66% / 33% / 0% (unsure about the 0%)
...
- When you run the `Touch Calibration` (Settings-2 screen), make sure to do all 4 corners properly! If you for example touch the middle of the screen instead of the bottom left corner and save, you will have a _bad_ time. You will then need to erase the ESP32 flash to remove the NVS calibration
  setting.
